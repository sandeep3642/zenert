...
class TransactionUtil extends Util
{
    ...
    private $hallmark_code;
    private $hallmark_price;
    private $hallmark_quantity;
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_customer == 1) {
            ...
            if ($receipt_printer_type != 'printer') {
                ...
                $output['customer_address'] = $customer->address_line_1;
                ...
                if (!empty($customer->landline)) {
                    ...
                    $output['customer_phone'] =  $customer->landline;
                }
            }
        ...
        $output['bank_details'] = Business::where('id', $business_details->id)->first();
        ...
        if ($transaction->discount_type == 'percentage') {
            ...
            $discount_unformatted = ($transaction->discount_amount / 100) * $transaction->total_before_tax;
        } else {
            $discount = $transaction->discount_amount;
            $discount_unformatted=$transaction->discount_amount;
        }
        $output['discount'] = ($discount != 0) ? $this->num_f($discount, $show_currency, $business_details) : 0;
        $output['discount_unformatted']=($discount != 0) ?$discount_unformatted:0;
        ...
        $output['tax_cgst_sgst'] = ($transaction->tax_amount != 0) ? $transaction->tax_amount : 0;
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            if($line->product_id=='76'){
                $this->hallmark_code=$line->product_id;
                $this->hallmark_price=$line->unit_price_inc_tax;
                $this->hallmark_quantity=$line->quantity;
