...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($contacts as $item) {
            $birthday = Carbon::parse($item->anniversary_date);
            $birthday->year(date('Y'));
            $diff = Carbon::now()->diffInDays($birthday, false);
            // if ($diff > -16 && $diff < 0 && $item->email_sent == 0) {
            //     $result = $this->notificationUtil->autoSendNotification($item->business_id, 'Anniversary', "Wishing a perfect pair a perfectly happy day", $item);
            // }
            // if ($result) {
            //     $item->email_sent = 1;
            //     $item->save();
        ...
        // $contacts = Contact::whereMonth('anniversary_date', '=', date('m'))->whereDay('anniversary_date', '<', date('d'), '<', 15)->get();
        // $currentTime = Carbon::now();
        // $contacts = Contact::whereDate('anniversary_date', $currentTime->addDays(15))->get();
        // foreach($contacts as $item){
        //     if($item->){
        ...
        return view('home.index', compact('contacts', 'date_filters', 'sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin'));
    }
