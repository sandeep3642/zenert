                 ...
                 <div class="col-md-12 col-sm-12 col-xs-12 col-custom">
        	        <div class="info-box info-box-new-style">
        	           <span class="info-box-icon bg-green">
            	        ...
            	        <div class="info-box-content">
            	          <span class="info-box-text">{{ __('home.anniversary') }}</span>
                          @foreach($contacts as $item)
                          <?php
                          //$totalDays=Carbon::now()->diffInDays(Carbon::parse($item->anniversary_date));
                          $birthday = Carbon::parse($item->anniversary_date);
                          $birthday->year(date('Y'));
                          $diff=Carbon::now()->diffInDays($birthday, false);
                          ?>
                          
                          {{-- Carbon::parse($item->anniversary_date)->diff(Carbon::now())->format('%y years, %m months and %d days') --}}
                              {{-- <p></p>   --}}
                              @if($diff>-16 && $diff<0)
                              <span>[{{$item->name." anniversary date remain ".abs($diff)." days"}}],</span>
                              @endif
                          @endforeach
            	          {{-- <span>[{{"No user anniversary date near"}}]</span> --}}
            	        </div>
            	        <!-- /.info-box-content -->
        	        </div>
        	      <!-- /.info-box -->
